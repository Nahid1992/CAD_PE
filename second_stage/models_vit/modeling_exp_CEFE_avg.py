...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, embedding=False):
        ...
        #                                stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, 512+10, config.hidden_size)) # n_patches+1 = 192+1? || hidden_size = 768
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids): # x
        ...
        encoded, attn_weights = self.encoder(embedding_output) # was embedding_output || now input_ids
        return encoded, attn_weights 


class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, HS=6144, MD=1024, NH=2, NL=2, zero_head=False, vis=False, embedding=False):
        ...
        self.head0 = Linear(config.hidden_size, 512)
        self.head1 = Linear(config.hidden_size, num_classes) # 1(B) x 1 x 768
    ...
    def forward(self, x, labels=None):
        x, attn_weights = self.transformer(x) # output X => B 193 768
        ...
        logits1 = self.head1(logits_clstoken[:,1]) # > 1 class output
