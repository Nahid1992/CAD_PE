...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, attn_weights = self.encoder(input_ids) # was embedding_output || now input_ids
        return encoded, attn_weights


class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, HS=6144, MD=1024, NH=2, NL=2, zero_head=False, vis=False):
    ...
    def forward(self, x, labels=None):
        ...
        # logits = self.head(x[:, 0]) # something is not right
        logits = self.head(x)
